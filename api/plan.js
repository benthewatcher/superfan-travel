/**
 * POST /api/plan
 * Body:  { origin: string, club: string }
 * Returns: { cards: [{ title, subtitle?, emoji?, link? }] }
 */
import OpenAI from 'openai';

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

/** Strip ``` fences & parse JSON safely */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('RAW GPT RESPONSE:', raw);      // <-- debug log INSIDE the handler
    ...
    // fallback: wrap raw text so UI still shows something useful
    return res.status(200).json({
      cards: [{ title: 'Trip Plan', subtitle: raw }]
    });
